<template>
    <div class="good-list" @scroll="listScroll" ref="mainroot">
        <li v-for="(item,key) in goods" :key="key" ref="list">
            <div class="catagory" :class="{actived:currentIndex === key}">{{item.name}}</div>
            <good-item class="good-item" v-for="(i,k) in item.items" :key="k" :good="i" @add-good="$emit('add-good',$event)"></good-item>
        </li>
    </div>
</template>
...
export default {
    ...
    watch:{
        selectIndex(newIndex,oldIndex) {
            ...
            this.$nextTick(()=>{
                root.scrollTop = this.heights[newIndex] - 150 < 0 ? 0 : this.heights[newIndex] - 150;
            });
        }
    },
    data() {
